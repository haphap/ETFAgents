...
_PORTFOLIO_MANAGER_FREETEXT_FALLBACK_INSTRUCTION = (
    ...
    "and format `（二）建议` with the five numbered paragraphs (初始仓位与执行节奏 / 加仓触发条件 / 减仓与止损条件 / 再平衡触发 / 后续验证指标) separated by blank lines as described above."
)
...
## {localize_label("Positioning Recommendation", "持仓建议")}
 ...
 - In `（二）建议`, write the execution guidance as separate numbered paragraphs with a blank line between each block. Use exactly this numbering and order:
   1. `初始仓位与执行节奏` — target allocation band, maximum initial sizing, and baseline stance.
