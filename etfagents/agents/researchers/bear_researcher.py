...
{"Your main argument body must be written entirely in Chinese." if get_output_language().strip().lower() in CHINESE_OUTPUT_VALUES else f"Write your main argument body in {get_output_language()}."} {get_bear_proposal_instruction()}
{get_analyst_decision_instruction()}
...
{get_snapshot_writing_instruction(round_index)}{get_language_instruction()}{get_no_greeting_instruction()}"""
        ...
        history_turn = build_history_turn(
            rebuild_visible_debate_turn(
                visible_argument_body, decision_summary, new_bear_snapshot_full
            ),
            "Bear Analyst",
        )
