...
def _looks_like_complete_top_holdings_report(report: str) -> bool:
    """Positive contract for accepting top-holdings research into graph state."""
    content = report or ""
    ...
    return (
        all(marker in content for marker in _TOP_HOLDINGS_REQUIRED_MARKERS)
...
def create_etf_stock_research_analyst(llm):
    def etf_stock_research_node(state):
        ...
        system_message = inject_memory_prompt_section((
            ...
            + get_topic_and_term_style_instruction() + "\n"
            + get_concise_heading_instruction() + "\n"
            "撰写全面的Markdown报告。每个一级章节（一、二、三、四）必须按'一级标题 -> 2-3句结论导语 -> 子章节标题'的顺序输出。"
            "章节导语必须是带券商证据、持仓权重或ETF归因含义的结论句，不是任务说明；"
        ...
        result, report = run_tool_report_chain(
            ...
            unexecuted_tool_recovery={
                ...
                "tool_payloads": [
                    ...
                    {
                        "tool": get_etf_top_holdings_research,
                        "payload": {"ticker": asset_symbol, "curr_date": current_date},
                    },
                ],
            },
