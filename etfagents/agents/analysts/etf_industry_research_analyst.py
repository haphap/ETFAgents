...
def create_etf_industry_research_analyst(llm):
    def etf_industry_research_node(state):
        ...
        system_message = inject_memory_prompt_section((
            ...
            "每个一级章节（一、二、三、四）必须按'一级标题 -> 1-2句结论段 -> 子章节标题'的顺序输出。"
            "标题后的结论段必须是带券商证据、ETF权重或配置含义的判断句，不是任务说明；"
            "第一章标题后的结论段必须从券商共识来源和分歧来源切入，不得复制或近似复述开篇帽段；"
            ...
            "- 开篇帽段只写全局主判断；第一章标题后的结论段必须改写为共识证据与分歧证据的摘要，不能与开篇帽段使用同一句或同一表达。\n"
