 ...
 *   4. Fail with a clear message asking the user to run `uv sync --frozen`.
 *
 ...
 * surface inside LangChain imports rather than at startup.
 */
...
/** Path to `etfagents/bridge/__main__.py` relative to the resolved repo root. */
const BRIDGE_MARKER = join("etfagents", "bridge", "__main__.py");
...
/** Resolve the directory that contains `etfagents/bridge/__main__.py`. */
export function findRepoRoot(): string {
  ...
  throw new BridgeStartupError(
    `Could not locate the ETFAgents repo root. Looked for ${BRIDGE_MARKER} ` +
...
export function resolvePython(repoRoot: string = findRepoRoot()): ResolvedPython {
  const envPath = process.env.ETFAGENTS_PYTHON;
  if (envPath && envPath.trim() !== "") {
    if (!isExecutableFile(envPath)) {
      throw new BridgeStartupError(
        `ETFAGENTS_PYTHON=${envPath} does not exist or is not a file. ` +
  ...
  throw new BridgeStartupError(
    `No Python interpreter found.\n` +
      `  - $ETFAGENTS_PYTHON is unset.\n` +
      `  - ${venvPosix} does not exist.\n` +
      `  - ${venvWindows} does not exist.\n` +
      `Run \`uv sync --frozen\` from ${repoRoot} to create the project venv, ` +
      `or set ETFAGENTS_PYTHON to a python that has etfagents installed.`,
  );
}
